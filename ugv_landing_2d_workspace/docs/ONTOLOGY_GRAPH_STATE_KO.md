...
실행 중에 이를 강제하고, `run_all`이 학습 전에 호출합니다.
...
### 4.6 현재 성능 (시드 3개, 같은 학습 예산)

기본 설정으로 시드 세 개(20240501 / 7 / 1234)에서 학습해 최종 선택된 정책을
공칭 초기 조건으로 평가한 결과입니다. 보상, 행동, 환경, 종료 조건, PPO 설정,
학습 반복 수는 두 비교군이 같습니다.

| 상태 표현 | 시드별 착륙률 | 평균 착륙률 | 평균 점수 | 평균 포착률 |
| --- | --- | --- | --- | --- |
| 기준 관측 벡터 | 100% / 100% / 100% | **100%** | **+125.4** | 86% |
| 온톨로지 그래프 | 0% / 67% / 100% | 56% | -20.8 | 83% |

**현재 제안 모델은 기준 모델보다 못하고, 시드에 따른 분산이 훨씬 큽니다.**
기준 모델은 세 시드 모두 안정적으로 착륙합니다.

가장 좋았던 실행(시드 1234)에서는 제안 모델의 착륙 시각이
28.8 / 41.6 / 40.7 s로 기준 모델(30.2 / 39.3 / 42.1 s)과 대등하거나 조금
빨랐습니다. 즉 이 표현으로도 좋은 정책에 도달할 수는 있지만, **안정적으로
도달하지 못합니다.**

원인으로 보이는 것은 두 가지입니다.

1. 4.4에서 적은 표현의 상한. 모드 플래그와 `PadMotion` 병합으로 잃은 정보가
   교사 모방 손실 0.0667 대 0.0071의 차이로 남아 있습니다.
2. 학습 중 표현이 움직입니다. 기준 모델의 상태는 고정된 함수이지만, 제안
   모델의 `g_t`는 부호기가 학습되면서 계속 바뀝니다. 정책과 가치망이 그
   움직이는 표현을 따라가야 하므로 학습이 불안정해집니다.

2번을 줄이려면 부호기 학습률(`gs.encoderLearnRate`, 기본 3e-4)을 낮추거나,
모방 학습 뒤 몇 반복 동안 부호기를 얼리는 방법을 시도해 볼 수 있습니다.
어느 쪽도 보상이나 PPO를 건드리지 않으므로 이 실험의 통제 조건을 깨지 않습니다.
아직 측정해 보지 않았습니다.
