...
| 상태 표현 | 최초 착륙 | 최고 평가 점수 | 결과 |
| --- | --- | --- | --- |
| 기준 관측 벡터 11차원 | **없음** | -38.2 | 2500반복 내내 착륙 0% |
| 온톨로지 그래프 | **550반복** | **+124.0** | 착륙 100% |

기준 관측 벡터로는 교사 없이 한 번도 착륙하지 못하고 포착률만 높은 국소 최적해에
갇히는 반면, 온톨로지 그래프 상태로는 550반복에서 착륙을 찾고 1100반복에서 착륙률
100%에 도달합니다. 이 점수는 **교사를 쓴 기준 모델(+125.4)과 거의 같은 수준**이며,
PN 유도를 한 번도 보지 않고 도달한 값입니다.
...
| 교사 없이 (PPO 최대 2500반복) | 약 900초 | 약 2~3시간 |

교사 없는 학습에는 조기 종료가 켜져 있습니다(`rl.earlyStopPatience = 15`).
평가 점수가 15회(375반복) 연속 최고점을 넘지 못하면 멈춥니다. `ppoTrain`이 최고
점수의 정책을 따로 보관하므로 **돌려주는 정책은 달라지지 않고 시간만 줄어듭니다.**
...
| 상태 표현 | 최초 착륙 | 최고 평가 점수 | 결과 |
| --- | --- | --- | --- |
| 기준 관측 벡터 | **없음** | -38.2 | 2500반복 내내 착륙 0% |
| 온톨로지 그래프 | **550반복** | **+124.0** | 착륙 100% |
...
제안 모델은 550반복에서 착륙을 찾았고 1100반복에서 착륙률 100%·점수 +124.0에
도달했습니다. 이는 **교사를 쓴 기준 모델(+125.4)과 거의 같은 수준**이며,
PN 유도를 한 번도 보지 않고 도달한 값입니다.
