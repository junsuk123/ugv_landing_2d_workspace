...
FirstReobservation_s = nan(rows,1);
TotalOcclusion_s = nan(rows,1);
TrackingRecovery_s = nan(rows,1);
...
CumulativeClimbAfterLoss_m = nan(rows,1);
row = 0;
for i = 1:numel(runs)
    for j = 1:numel(runs(i).results)
        ...
        FirstReobservation_s(row) = recovery.firstReobservation_s;
        TotalOcclusion_s(row) = recovery.totalOcclusion_s;
        TrackingRecovery_s(row) = recovery.trackingRecovery_s;
        ...
        CumulativeClimbAfterLoss_m(row) = recovery.cumulativeClimbAfterLoss_m;
...
summaryTable = table(Controller,Scenario,FirstLoss_s,FirstReobservation_s, ...
    FirstReacquire_s,ReacquireDelay_s,TrackingRecovery_s,TotalOcclusion_s, ...
    ClimbAfterLoss_m,CumulativeClimbAfterLoss_m,Landing_s, ...
    PeakHeightAbovePad_m,PadCaptureRate,Status);
end
