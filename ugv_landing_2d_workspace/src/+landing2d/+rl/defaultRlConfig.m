...
%
% 조기 종료로 학습 시간을 줄이려 했지만, 실제 학습 곡선을 보고 철회했습니다.
% 온톨로지 그래프 상태로 교사 없이 2500반복을 돌린 기록입니다.
%
%   550반복   최초 착륙
%   1100반복  점수 124.00 (착륙 100%)
%   ~2300반복까지 이 점수를 넘지 못함
%   2375반복  점수 151.38 (착륙 100%)  <- 최종 선택된 정책
%
% 1100에서 2375까지 **평가 51회 연속으로 최고점을 넘지 못했습니다.** patience를
% 15(평가 15회)로 두었다면 1475반복 근처에서 멈춰 마지막 최고점을 통째로 놓쳤을
% 것입니다. 이 문제의 학습 곡선은 정체 구간이 길고 뒤늦게 크게 좋아지므로,
% "정체하면 멈춘다"는 규칙이 성능을 깎습니다.
%
% 기능 자체는 남겨 두었으니, 학습 곡선을 이미 확인한 설정에서 시간을 아끼고 싶을 때만
% 충분히 큰 값(평가 60회 이상)으로 켜십시오. 켜면 지문이 달라져 다시 학습합니다.
