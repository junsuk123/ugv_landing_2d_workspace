...
> **재현성 주의:** 접근 보상의 거리 변화율이 `d_(t-1)-d_t`가 아니라 `0-d_t`로 계산되던 오류를 수정했습니다. 수정 전 정책과 결과 수치는 자동으로 무효화되며, 새 알고리즘 버전으로 재학습·재평가해야 합니다.
...
| Semantic-flat PPO | 그래프와 동일한 의미 특징을 평탄화한 벡터 | 정보 추가 효과를 그래프 구조 효과와 분리하는 대조군 |
...
관측 기억에는 마지막 패드 속도뿐 아니라 관측으로 추정한 패드 가속도, FOV 여유 변화율, 경계 도달 긴급도, 미관측 시간에 따른 불확실성, 상대 고도 문맥을 유지합니다. 가려진 동안 실제 UGV 위치·속도·미래 명령은 사용하지 않습니다.

수평 추종만으로 시야를 유지할 수 있는지 판단하는 이상적 진단 경계도 매 실행 시 출력합니다.

$$
e_{\max}=\frac{\Delta v^2}{2}\left(\frac{1}{a_{\mathrm{drone}}}-\frac{1}{a_{\mathrm{UGV}}}\right),
\qquad
h_{\min}=\frac{e_{\max}}{\tan(\theta)}
$$

기본 설정에서는 S1은 초기 고도 6 m에서 수평 대응만으로 유지 가능하지만, S2·S3은 이상적인 최대 가속을 가정해도 시야 유지가 불가능합니다. 따라서 상승 자체를 실패로 해석하지 않고, 불필요한 상승량과 재포착·추종 회복 시간을 별도로 평가합니다.
...
노드 $i$의 의미 값 $v_{i,t}$, 위험 노드 여부 $q_i$, 방향 부호 $d_{i,t}$, 동적 문맥 $c_{i,t}\in\mathbb{R}^5$와 노드 정체성 one-hot 벡터 $e_i$를 결합합니다.
...
\end{bmatrix},
\qquad
X_t=[x_{1,t},\ldots,x_{N,t}]\in\mathbb{R}^{(10+N)\times N}
$$

현재 $N=9$이므로 각 노드 특징은 19차원입니다. $v_{i,t}$는 위험 또는 안전의 크기이고, $d_{i,t}\in[-1,1]$는 절댓값 기반 의미 값에서 사라지는 좌·우 및 상승·하강 방향을 복원합니다. $c_{i,t}$는 패드 가속 추세, FOV 여유 변화율, 경계 긴급도, 기억 불확실성, 고도 문맥을 관련 노드에만 배치합니다. 모든 값은 관측·관측 기억·드론 자기 상태만 사용합니다.
...
- 총 시야 이탈 시간, 최초 재관측, 안정 재포착, 추종 회복, 상승량, 착륙 이벤트 평가
- 알고리즘 버전을 포함한 설정 지문 기반 정책 재사용과 MATLAB 회귀 테스트
...
- 시간축 부호 수평오차와 ±FOV 경계, 패드 상대 회복 궤적 및 이벤트
